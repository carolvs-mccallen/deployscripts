...
add_repositories() {
  echo "Adding common repositories..."
  ...
  dnf -y install --best --allowerasing epel-release epel-next-release
...
install_packages() {
  ...
  dnf -y install --best --allowerasing "${packages[@]}"
...
dnf -y install --best --allowerasing arj azure-cli brave-browser cabextract cifs-utils deja-dup digikam dnf-utils dpkg gcc-go gh gnupg2-smime golang-bin google-carlito-fonts google-crosextra-caladea-fonts htop hyphen-en hyphen-es innoextract kdiff3 kdiskmark kleopatra krename krusader ksystemlog ktorrent kubectl libcurl-devel libdrm-devel libpciaccess-devel libxml2-devel lzma microsoft-edge-stable mythes mythes-en mythes-es neofetch nextcloud-client nextcloud-client-dolphin nfs-utils openssl-devel okteta perl podman podman-docker powertop pstoedit python3-pip thunderbird tlp tracker unrar vim-enhanced xkill
