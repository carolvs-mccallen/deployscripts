...
dnf install --best --allowerasing -y arj azure-cli brave-browser cabextract deja-dup digikam dnf-utils dpkg gcc-go gh gnupg2-smime golang-bin htop httpd-tools innoextract kate kamoso kdiff3 kdiskmark kleopatra krename krusader ksystemlog ktorrent kubectl libcurl-devel libdrm-devel libpciaccess-devel libxml2-devel lzma microsoft-edge-stable neofetch nextcloud-client nextcloud-client-dolphin openssl-devel okteta perl podman-docker powertop pstoedit python3-pip thunderbird tlp tracker unrar vim-enhanced xkill
