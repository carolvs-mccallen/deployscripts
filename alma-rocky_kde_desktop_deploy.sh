...
dnf -y install --best --allowerasing arj azure-cli brave-browser cabextract cifs-utils deja-dup digikam dnf-utils dpkg gcc-go gh gnupg2-smime golang-bin google-carlito-fonts google-crosextra-caladea-fonts htop hyphen-en hyphen-es innoextract kdiff3 kdiskmark kleopatra krename krusader ksystemlog ktorrent kubectl libcurl-devel libdrm-devel libpciaccess-devel libxml2-devel lzma microsoft-edge-stable mythes mythes-en mythes-es neofetch nextcloud-client nextcloud-client-dolphin nfs-utils openssl-devel okteta perl podman podman-docker pstoedit python3-pip thunderbird tracker unrar vim-enhanced xkill
