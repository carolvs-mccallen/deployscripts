...
apt install -f -y
rm *.deb
apt install --install-recommends -y alien apt-transport-https arj azure-cli brave-browser cheese deja-dup doublecmd-gtk eog eog-plugins epiphany-browser epiphany-browser-data evolution frei0r-plugins firefox-locale-es gimp gimp-data-extras gimp-help-common gimp-help-en gimp-help-es git git-cvs git-daemon-run git-doc git-email git-gui git-mediawiki git-svn gitk gnome-contacts gnome-mahjongg gnome-maps gnome-mines gnome-video-effects-frei0r gnome-weather gpa hashdeep hyphen-es hyphen-fi hyphen-ga hyphen-id kid3-qt language-pack-es language-pack-gnome-es lhasa libdvd-pkg libfprint-2-dev libfprint-2-doc libmypaint-1.5-1 libmypaint-common libncurses5 libncurses5:i386 libpam-fprintd linux-generic-hwe-22.04 linux-headers-generic-hwe-22.04 linux-image-generic-hwe-22.04 malcontent-gui mint-meta-codecs mythes-es nautilus-dropbox nemo-nextcloud nextcloud-desktop nextcloud-desktop-common nextcloud-desktop-doc nextcloud-desktop-l10n nfs-common openclipart-libreoffice openoffice.org-hyphenation pstoedit python3-pydrive rar rpm rpm-i18n signal-desktop traceroute uget unace unrar-free vim vlc xboxdrv
